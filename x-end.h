...
xaxis_end_motor_offset=[-xaxis_end_motorsize/2-zaxis_bearing_OD/2+3*mm,motor_mount_wall_thick-2*mm,0];
xaxis_end_pulley_offset = xaxis_end_motor_offset[0]+10*mm;
xaxis_end_wz = xaxis_rod_distance+xaxis_rod_d+10*mm;

function xaxis_endstop_pos(with_motor) = [xaxis_end_width_right(with_motor),xaxis_rod_d/2+8*mm,-5*mm];

xaxis_endstop_size_switch = [10.3*mm, 6.3*mm, 20*mm];
